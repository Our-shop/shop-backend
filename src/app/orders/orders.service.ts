...
export class OrdersService {
    ...
    private async createOrderItemsForOrder(newOrder: NewOrderDto, createdOrder: OrderEntity) {
    ...
    private async checkProductsQuantity(newOrder) {
        return Promise.all(newOrder.products.map(async (product) => {
            const productInStock = ProductDto.fromEntity(await this.productsRepo.getById(product.id));
