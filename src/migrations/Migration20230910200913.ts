...
export class Migration20230910200913 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table "products" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "status" text check ("status" in (\'active\', \'archived\')) not null default \'active\', "title" varchar(255) not null, "price" double precision not null, "description" varchar(255) not null, "image" varchar(255) not null, "quantity" int not null, "category" text check ("category" in (\'food\', \'clothes\', \'toys\')) not null, "type" text check ("type" in (\'dry-food\', \'sticks\', \'ball\', \'sound-toy\', \'sweater\', \'costume\')) not null, "size" varchar(255) null, "expiration_date" varchar(255) null, "recommended_date" varchar(255) null, constraint "products_pkey" primary key ("id"));');
    ...
    this.addSql('create table "refresh_tokens" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "Refresh_token" varchar(1000) not null, "user_id" uuid not null, constraint "refresh_tokens_pkey" primary key ("id"));');
