...
export class Migration20230823160726 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "orders" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "status" text check ("status" in (\'active\', \'archived\')) not null default \'active\', "total_amount" int not null, "delivery_id" uuid null, "user_id" uuid null, constraint "orders_pkey" primary key ("id"));');
    ...
    this.addSql('create table "carts" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "status" text check ("status" in (\'active\', \'archived\')) not null default \'active\', "user_id" uuid null, "discount" int not null, constraint "carts_pkey" primary key ("id"));');

    this.addSql('create table "cart_items" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "status" text check ("status" in (\'active\', \'archived\')) not null default \'active\', "cart_id" uuid null, "product_id" varchar(255) not null, "product_quantity" int not null, constraint "cart_items_pkey" primary key ("id"));');
    ...
    this.addSql('alter table "carts" add constraint "carts_user_id_foreign" foreign key ("user_id") references "users" ("id") on update cascade on delete set null;');

    this.addSql('alter table "cart_items" add constraint "cart_items_cart_id_foreign" foreign key ("cart_id") references "carts" ("id") on update cascade on delete set null;');
  }
